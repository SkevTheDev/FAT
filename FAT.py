...
import datetime
sys.path.insert(1, './Models/')  # python only searches for files in local folder
sys.path.insert(1, './Utils/')  # python only searches for files in
...
def train(model, train_loader, optimizer, tau):
    ...
    for batch_idx, (data, target) in enumerate(train_loader):
        ...
        #loss_sum += loss.item()  #.detach().item()
...
if args.dataset == "cifar10":
    ...
    train_loader = torch.utils.data.DataLoader(trainset, batch_size=128, shuffle=True, num_workers=0)  #num_workers=2 will not work on windows 10
